...
def make_cfg(tmp_path):
    def build(**overrides):
        data = {"atlas_dir": str(tmp_path / "atlas"), "repo_roots": [str(tmp_path / "src")]}
...
def make_manifest(tmp_path):
    def build(cfg, name, **fields):
        m = {
            "name": name, "summary": f"{name} summary", "overview": "", "domain": "unassigned",
            ...
            "_meta": {"repo_path": str(tmp_path / "src" / name), "commit": "0" * 40,
                      "generated_at": "2026-01-01T00:00:00+00:00", "mode": "full", "model": "m",
                      "last_full_at": "2026-01-01T00:00:00+00:00", "dropped_without_evidence": []},
        }
        m.update(fields)
...
def graph_of(make_cfg):
    def build(cfg, known=None):
        names = known if known is not None else {
            p.stem for p in (cfg["atlas_dir"] / "repos").glob("*.json")}

...
def test_other_kind_exposes_can_be_matched(make_cfg, make_manifest, graph_of):
    cfg = make_cfg()
    make_manifest(
        cfg, "dropbox-svc", exposes=[{"kind": "sftp", "name": "drop zone", "key": "drop-zone", "evidence": ev()}]
    )
    make_manifest(
        cfg, "importer", consumes=[{"kind": "sftp", "name": "drop zone", "key": "drop-zone", "evidence": ev("b.go")}]
    )
    g = graph_of(cfg)
    assert [(e["from"], e["to"], e["match"]) for e in g["edges"]] == [("importer", "dropbox-svc", "exact")]
    assert g["unresolved"] == []


def test_ambiguous_alias_above_cap_becomes_unresolved_with_candidates(make_cfg, make_manifest, graph_of):
    cfg = make_cfg(max_ambiguous_hits=3)
    for i in range(4):
        make_manifest(cfg, f"orders-{i}", identifiers=[f"orders.cell{i}.internal"])
    make_manifest(cfg, "client", consumes=[{"kind": "http", "name": "orders", "key": "orders", "evidence": ev()}])
    g = graph_of(cfg)
    assert g["edges"] == []
...
def test_alias_within_cap_still_produces_edges(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(cfg, "client", consumes=[{"kind": "http", "name": "orders", "key": "orders", "evidence": ev()}])
    g = graph_of(cfg)
    assert [(e["from"], e["to"], e["match"]) for e in g["edges"]] == [("client", "orders-a", "alias")]
...
def test_envvar_consume_resolves_to_the_named_service(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(
        cfg, "client", consumes=[{"kind": "http", "name": "orders", "key": "ORDERS_SERVICE_URL", "evidence": ev()}]
    )
    g = graph_of(cfg)
    assert [(e["from"], e["to"], e["match"]) for e in g["edges"]] == [("client", "orders-service", "envvar")]
...
def test_exact_host_match_beats_envvar_tier(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(
        cfg, "client", consumes=[{"kind": "http", "name": "orders", "key": "orders-service.internal", "evidence": ev()}]
    )
    g = graph_of(cfg)
...
def test_owned_datastore_links_database_consumers(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(
        cfg,
        "reporting",
        consumes=[{"kind": "database", "name": "orders db", "key": "orders_db", "evidence": ev("b.go")}],
    )
    g = graph_of(cfg)
...
def test_generic_datastore_name_is_not_indexed(make_cfg, make_manifest, graph_of):
    cfg = make_cfg()
    make_manifest(cfg, "a", datastores=[{"kind": "postgres", "name": "postgres", "access": "owner", "evidence": ev()}])
    make_manifest(cfg, "b", consumes=[{"kind": "database", "name": "pg", "key": "postgres", "evidence": ev("b.go")}])
    assert graph_of(cfg)["edges"] == []


def test_non_owner_datastore_does_not_provide(make_cfg, make_manifest, graph_of):
    cfg = make_cfg()
    make_manifest(cfg, "a", datastores=[{"kind": "postgres", "name": "orders_db", "access": "read", "evidence": ev()}])
    make_manifest(cfg, "b", consumes=[{"kind": "database", "name": "o", "key": "orders_db", "evidence": ev("b.go")}])
    assert graph_of(cfg)["edges"] == []
...
def test_external_package_dependency_is_ignored(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(
        cfg,
        "web",
        packages={"publishes": [], "depends_on": [{"ecosystem": "npm", "name": "react", "evidence": "package.json"}]},
    )
    g = graph_of(cfg)
    assert g["edges"] == []
    assert g["unresolved"] == []
...
def test_same_bare_package_name_in_two_ecosystems_stays_alias_tier(make_cfg, make_manifest, graph_of):
    cfg = make_cfg()
    make_manifest(
        cfg,
        "py-lib",
    ...
    make_manifest(
        cfg,
        "js-app",
        packages={"publishes": [], "depends_on": [{"ecosystem": "npm", "name": "widgets", "evidence": "package.json"}]},
    )
    g = graph_of(cfg)
...
def test_orphan_manifests_are_excluded_from_the_graph(make_cfg, make_manifest, graph_of, capsys):
    ...
    make_manifest(cfg, "client", consumes=[{"kind": "http", "name": "d", "key": "deleted.internal", "evidence": ev()}])
...
def test_shared_datastores_lists_stores_touched_by_more_than_one_repo(make_cfg, make_manifest, graph_of):
    cfg = make_cfg()
    make_manifest(cfg, "a", datastores=[{"kind": "postgres", "name": "orders_db", "access": "owner", "evidence": ev()}])
    make_manifest(
        cfg, "b", datastores=[{"kind": "postgres", "name": "orders_db", "access": "read", "evidence": ev("b.go")}]
    )
    make_manifest(
        cfg, "c", datastores=[{"kind": "postgres", "name": "solo_db", "access": "owner", "evidence": ev("c.go")}]
    )
    g = graph_of(cfg)
...
def test_package_path_identifier_matches_only_its_own_repo(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(
        cfg, "client", consumes=[{"kind": "package", "name": "a", "key": "github.com/org/a", "evidence": ev()}]
    )
    g = graph_of(cfg)
...
def test_code_host_url_does_not_link_every_go_repo(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(cfg, "go-b", identifiers=["github.com/org/b"])
    make_manifest(
        cfg, "client", consumes=[{"kind": "http", "name": "repo", "key": "https://github.com/org/a", "evidence": ev()}]
    )
    g = graph_of(cfg)
    assert g["edges"] == []
...
def test_scoped_package_identifier_does_not_provide_its_scope(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(cfg, "client", consumes=[{"kind": "http", "name": "org", "key": "org", "evidence": ev()}])
    assert graph_of(cfg)["edges"] == []


def test_scoped_package_identifier_still_matches_a_package_consume(make_cfg, make_manifest, graph_of):
    cfg = make_cfg()
    make_manifest(cfg, "shared", identifiers=["@org/shared"])
    make_manifest(
        cfg, "client", consumes=[{"kind": "package", "name": "shared", "key": "@org/shared", "evidence": ev()}]
    )
    assert [(e["from"], e["to"], e["match"]) for e in graph_of(cfg)["edges"]] == [("client", "shared", "exact")]
...
def test_too_many_exact_hits_are_capped_with_candidates(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(cfg, "client", consumes=[{"kind": "http", "name": "o", "key": "orders.internal", "evidence": ev()}])
    g = graph_of(cfg)
    assert g["edges"] == []
...
def test_many_producers_of_one_topic_still_link(make_cfg, make_manifest, graph_of):
    ...
    for i in range(4):
        make_manifest(
            cfg,
            f"producer-{i}",
            exposes=[{"kind": "topic", "name": "OrderCreated", "key": "order.created", "evidence": ev()}],
        )
    make_manifest(
        cfg, "consumer", consumes=[{"kind": "topic", "name": "OrderCreated", "key": "order.created", "evidence": ev()}]
    )
    g = graph_of(cfg)
...
def test_a_generically_named_repo_does_not_attract_alias_matches(make_cfg, make_manifest, graph_of):
    ...
    make_manifest(
        cfg,
        "client",
        consumes=[{"kind": "http", "name": "orders", "key": "https://api.orders.internal/v1", "evidence": ev()}],
    )
    g = graph_of(cfg)
    assert g["edges"] == []
...
def test_a_generic_envvar_stem_does_not_attract_matches(make_cfg, make_manifest, graph_of):
    cfg = make_cfg()
    make_manifest(cfg, "api")
    make_manifest(cfg, "client", consumes=[{"kind": "http", "name": "api", "key": "API_URL", "evidence": ev()}])
    assert graph_of(cfg)["edges"] == []

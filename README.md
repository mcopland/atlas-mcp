...
- `repo_names`: pin a name for a clone, `{"/abs/path/to/clone": "orders-service"}`. Needed only if you want a name that differs from the directory, or want it stable regardless of what else gets cloned. The name becomes a file name, so it must be letters, digits, `.`, `_` and `-`, starting with a letter or digit.
...
Both modes redact before the model sees anything: private key blocks (PEM and PGP), assignments to secret-looking keys, credentials inside URLs, passwords and account keys inside connection strings, tokens whose provider prefix gives them away (AWS access key ids, GitHub, GitLab, Slack, Google, npm, Stripe, Anthropic, OpenAI, JWTs), and long opaque quoted literals become `<redacted>`. The prompts tell the model never to copy a secret value into an entry, and every string the model writes back goes through the same redaction before the entry is saved. Only regular files inside the repo are read: a committed symlink that points outside it is skipped. Redaction is a safety net over the fact that the bundle prefers `.env.example` over `.env`, not a licence to point the mapper at a repo full of live keys.
...
- **Mapping runs go through a dedicated agent.** Without one, `kiro-cli` maps each repo under that repo's own workspace configuration, which means starting whatever MCP servers and hooks the repo declares, unattended, with your environment and `KIRO_API_KEY`. `atlas-mapper` (explore mode) sets `includeMcpJson: false`, no MCP servers, and read-only tools, and its `permissions` deny reads of credential locations (`.ssh`, `.aws`, `.kube`, `.config`, `.kiro`, `.netrc`, real `.env` files, key files) wherever they are, so a repo whose README tells the model to read `~/.aws/credentials` gets a refusal rather than the file. The globs are written as `**/.aws/**` rather than `~/.aws/**` because Kiro does not document `~` expansion in `match`; if you add your own, confirm with a pilot run that the read is refused. `atlas-bundle` (bundle mode) declares no tools at all, and its session runs from `atlas_dir` rather than inside the mapped repo, so none of that repo's configuration is loaded in the first place. Kiro still inherits default resources such as workspace steering unless you set `chat.disableInheritingDefaultResources`; that is text in the prompt, not code that runs.
